...
class FashionTrainer:
    ...
    def _initialize_models(self, finetune_clip: bool = False, finetune_layers: int = 2):
        ...
        print(
            f"  Image Encoder: {'FashionCLIP (fashion-specific)' if self.using_fashionclip else 'Standard CLIP'}"
        )
        print(
            f"  Fine-tuning: {'Enabled (last {} layers)'.format(finetune_layers) if finetune_clip else 'Disabled (frozen)'}"
        )
        print(
            f"  JSON Encoder parameters: {sum(p.numel() for p in self.json_encoder.parameters()):,}"
        )
        print(
            f"  CLIP Encoder parameters: {sum(p.numel() for p in self.clip_encoder.parameters()):,}"
        )
    ...
    def _initialize_training_components(self):
        ...
        self.scheduler = optim.lr_scheduler.CosineAnnealingLR(
            self.optimizer, T_max=self.config.max_epochs, eta_min=self.config.learning_rate * 0.01
        )
    ...
    def train_json_encoder_standalone(
        self, train_loader: DataLoader, val_loader: DataLoader, num_epochs: int = 5
    ) -> Dict[str, Any]:
        """
        ...
        for epoch in range(num_epochs):
            ...
            print(
                f"  Output Stats: mean={val_stats['mean']:.4f}, std={val_stats['std']:.4f}, norm={val_stats['norm']:.4f}"
            )
    ...
    def _validate_json_encoder_standalone(
    ...
    def _analyze_output_distribution(self, val_loader: DataLoader) -> Dict[str, Any]:
        ...
        analysis = {
            ...
            "is_normalized": torch.allclose(
                torch.norm(all_embeddings, dim=-1), torch.ones(all_embeddings.size(0)), atol=1e-3
            ),
        }
    ...
    def train_contrastive_learning(
        ...
        Args:
            train_loader: Training data loader
            val_loader: Validation data loader (optional, can be None for speed)
        ...
        if val_loader:
            ...
            print(f"Validation: SKIPPED (speed mode)")
        ...
        # Final evaluation
        final_metrics = self._final_evaluation(val_loader) if val_loader is not None else {}
    ...
    def _train_epoch(self, train_loader: DataLoader, epoch: int) -> float:
        """Train for one epoch with Mixed Precision (AMP) support."""
        ...
        for batch_idx, batch in enumerate(train_loader):
            ...
            # Handle both dict and ProcessedBatch types
            if isinstance(batch, dict):
                images = batch["image"]
                json_batch = batch["json_encoding"]
            else:
                images = batch.images
                json_batch = self._convert_batch_to_dict(batch)
            ...
            # Log every 100 batches with time estimate
            if batch_idx % 100 == 0:
                ...
                print(
                    ...
                    f"Time: {elapsed_minutes:.1f}m elapsed, ETA: {eta_minutes:.1f}m"
                )
        ...
        print(
            f"\n  Epoch completed in {total_time/60:.1f} minutes (avg {total_time/num_batches:.2f}s/batch)"
        )
    ...
    def _validate_epoch(self, val_loader: DataLoader, epoch: int) -> Tuple[float, Dict[str, float]]:
        ...
        with torch.no_grad():
            for batch in val_loader:
                ...
                embeddings = self.contrastive_learner.get_embeddings(images, json_batch)
    ...
    def _compute_validation_metrics(self, similarity_matrix: torch.Tensor) -> Dict[str, float]:
        ...
        top5_correct = (
            (top5_indices == torch.arange(batch_size).unsqueeze(1)).any(dim=1).float().mean()
        )

        # Mean reciprocal rank
        ranks = (
            similarity_matrix.argsort(dim=1, descending=True)
    ...
    def _should_early_stop(
        self, val_losses: list, patience: int = 10, min_delta: float = 1e-4
    ) -> bool:
    ...
    def _move_batch_to_device(self, batch):
        ...
        Handles both ProcessedBatch objects and dict types.
        """
        # Handle dict type (from custom collate_fn)
        if isinstance(batch, dict):
            result = {}
...
def create_trainer_from_data_module(
