...
from .data_models import ProcessedBatch
...
class FashionDataset(Dataset):
    ...
    def __init__(
        ...
        # Allow empty file_paths for validation when train_split=1.0
        if not file_paths:
            print("Warning: Empty dataset created (train_split=1.0)")
        ...
        if max_sequence_lengths is None:
            self.max_sequence_lengths = {"style": 10, "material": 10, "detail": 15}
        else:
...
def _pad_sequences(sequences: List[List[int]]) -> Dict[str, torch.Tensor]:
    ...
    return {"ids": padded_ids, "mask": mask}
...
def create_fashion_dataloader(
    ...
    if drop_last is None:
        drop_last = (
            shuffle  # True for training (shuffle=True), False for validation (shuffle=False)
        )
...
def create_augmented_transforms(
    image_size: int = 224, augment_prob: float = 0.5
) -> transforms.Compose:
    """
...
class FashionDataModule:
    ...
    def setup(self) -> None:
        ...
        # If train_split is 1.0, use all data for training and create empty val set
        ...
        train_transforms = create_augmented_transforms(
            image_size=self.image_size, augment_prob=self.augment_prob
        )
        ...
        self.train_dataset = FashionDataset(
            dataset_loader=train_loader, file_paths=train_paths, image_transforms=train_transforms
        )

        # Only create val_dataset if there are validation paths
        if val_paths:
            self.val_dataset = FashionDataset(
                dataset_loader=val_loader, file_paths=val_paths, image_transforms=val_transforms
            )
        else:
            # Create empty dataset for validation
            self.val_dataset = FashionDataset(
                dataset_loader=val_loader, file_paths=[], image_transforms=val_transforms
            )
    ...
    def get_sample_batch(self) -> ProcessedBatch:
        ...
        sample_loader = create_fashion_dataloader(
            dataset=self.train_dataset, batch_size=4, shuffle=False, num_workers=0
        )
